...
function HistoryPage() {
  ...
  const activeRunId =
    selectedRunId ?? runs.data?.find((run) => run.status === "completed")?.id ?? null;
    ...
    <>
        ...
        <div className="panel overflow-x-auto">
          <table className="w-full min-w-[48rem] border-collapse text-sm">
            ...
            <tbody className="divide-y divide-border">
              {(runs.data ?? []).map((run) => (
                <tr key={run.id} className={run.id === activeRunId ? "bg-secondary/50" : undefined}>
                  <td className="px-3 py-2 font-medium">{run.label}</td>
                  <td className="num px-3 py-2 text-muted-foreground">{dateTime(run.startedAt)}</td>
                  ...
                  <td className="num px-3 py-2 text-right font-semibold">
                    {run.status === "completed" ? money(run.recoverableTotal, run.currency) : "—"}
                  </td>
      ...
      {activeRun ? (
        <section className="mt-8">
          <h2 className="mb-3 text-sm font-semibold">Exceptions in {activeRun.label}</h2>
